...
function matrixMultiplication(discordMessage, msg ) {
    //need an algo to simplify the inputs. Instead of posting arrays
        ...
        if (discordMessage.length > 7) {
            ...
            } else if (discordMessage.search(matmulOpt[1]) >= 0 && discordMessage.search(matmulOpt[0]) < 0) {
                //only -m2 input
            ...
            } else if (discordMessage.search(matmulOpt[0]) >= 0 && discordMessage.search(matmulOpt[1]) >= 0) {
                //both -m1 -m2 input
